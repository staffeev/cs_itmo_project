...
Создадим скрипт `lab/mapred_script.py` и запишем туда следующее:
...
Как можно увидеть, задача выполнилась успешно и данные в `lab/output.tsv` сохранены.

#### 2.3 Визуализация

Напишем скрипт, который будет находить рисовать графики прибыли по разным регионам для категории с наибольшей суммарной прибылью. Будем использовать библиотеки `pandas` для манипуляции данными и `matplotlib` для рисования графиков.
